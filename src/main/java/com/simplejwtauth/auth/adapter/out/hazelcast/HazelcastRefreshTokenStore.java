...
public class HazelcastRefreshTokenStore implements RefreshTokenStore {
    ...
    public static final String TOKEN_USER_MAP_NAME = "simple-jwt-auth:rt-token-user";
    public static final String USER_TOKEN_MAP_NAME = "simple-jwt-auth:rt-user-token";
    ...
    @Override
    public RotateResult rotate(String plainOldToken, String plainNewToken) {
        String oldHash = sha256(plainOldToken);
        String userId = tokenToUser.remove(oldHash);
        if (userId == null) return new Invalid(); // 이미 회전됐거나 만료됨

        RefreshEntry oldEntry = userToToken.remove(userId);
        if (oldEntry == null) return new Invalid();
    ...
    @Override
    public String findUserId(String plainToken) {
        if (plainToken == null || plainToken.isBlank()) return null;
        return tokenToUser.get(sha256(plainToken));
    }
